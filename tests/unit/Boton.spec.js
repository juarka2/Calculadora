...
describe("Boton.vue", () => {
  ...
  it("spanX and spanY are integer", () => {
    const wrapper = shallowMount(Boton);
    expect(
      Number.isInteger(wrapper.vm.spanX) && Number.isInteger(wrapper.vm.spanY)
    ).toBe(true);
  });
  it("height and width match required formula", () => {
    const wrapper = shallowMount(Boton);
    const findWidth = `width: ${wrapper.vm.spanX * 45 +
      2 * (wrapper.vm.spanX - 1)}px`;
    const findHeight = `height: ${wrapper.vm.spanX * 45 +
      2 * (wrapper.vm.spanX - 1)}px`;
    expect(
      wrapper
        .find("button")
